...
//patch GPU memory capacity
func patchGPUMemoryCapacity(gpuMemCapacity []int) error {
	node, err := clientset.CoreV1().Nodes().Get(nodeName, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	if err != nil {
		...
		log.Infof("Updated  %s successfully.", memCapacity)
	}
	return err
}

// patch GPU status
func patchGPUStatus(gpuStatus []GPUStatus) error {
	node, err := clientset.CoreV1().Nodes().Get(nodeName, metav1.GetOptions{})
	if err != nil {
		return err
	}
	log.Infof("debug: gpuStatus is %+v", gpuStatus)
	gpuStatusJson, _ := json.Marshal(gpuStatus)
	log.Infof("debug: patch gpuStatusJson is %v", gpuStatusJson)
	if val, ok := node.Annotations[gpuInfo]; ok {
		if val == string(gpuStatusJson) {
			log.Infof("No need to update %s", gpuInfo)
			return nil
		}
	}

	newNode := node.DeepCopy()
	newNode.Annotations[gpuInfo] = string(gpuStatusJson)

	_, _, err = nodeutil.PatchNodeStatus(clientset.CoreV1(), types.NodeName(nodeName), node, newNode)
	if err != nil {
		log.Infof("Failed to update %s.", gpuInfo)
	} else {
		log.Infof("Updated  %s successfully.", gpuInfo)
	}
	return err
}

//patch GPU memory used
func patchGPUMemoryUsed(gpuMememUsed []int) error {
	node, err := clientset.CoreV1().Nodes().Get(nodeName, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	_, _, err = nodeutil.PatchNodeStatus(clientset.CoreV1(), types.NodeName(nodeName), node, newNode)
	if err != nil {
		log.Infof("Failed to update %s.", memUsed)
	} else {
...
//patch has processes
func patchHasProcesses(hasProcess string) error {
	log.Infof("# : patch has gpu process: %v", hasProcess)
	node, err := clientset.CoreV1().Nodes().Get(nodeName, metav1.GetOptions{})
	if err != nil {
		return err
	}
	newNode := node.DeepCopy()
	newNode.Labels[hasProcesses] = hasProcess
	_, _, err = nodeutil.PatchNodeStatus(clientset.CoreV1(), types.NodeName(nodeName), node, newNode)
	if err != nil {
		log.Infof("Failed to update has gpu processes %s.", string(hasProcesses))
	} else {
		log.Infof("Updated has gpu processes %s successfully.", string(hasProcesses))
	}
	return err
}
...
func patchProcesses(GPUProcess [][]ProcessDetail) error {
	...
	if string(GPUProcessJson) == "null" || len(string(GPUProcessJson)) == 0 || string(GPUProcessJson) == "" {
		newNode.Labels[hasProcesses] = "false"
	} else {
		newNode.Labels[hasProcesses] = "true"
	}
