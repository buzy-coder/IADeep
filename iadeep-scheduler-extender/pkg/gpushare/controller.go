...
func NewController(clientset *kubernetes.Clientset, kubeInformerFactory kubeinformers.SharedInformerFactory, tuningQueue workqueue.RateLimitingInterface, stopCh <-chan struct{}) (*Controller, error) {
	...
	c.schedulerCache = cache.NewSchedulerCache(c.nodeLister, c.podLister, tuningQueue)
...
func (c *Controller) syncPod(key string) (forget bool, err error) {
	...
	default:
		if utils.IsCompletePod(pod) {
			...
			// Clear etcd data here
			id := utils.GetGPUIDFromAnnotation(pod)
			nodeName := pod.Spec.NodeName
			podNames, err := methods.GetPodContentByEtcd(nodeName, strconv.Itoa(id))
			if err != nil {
				log.Printf("Error: Get content err due to %+v", err)
			}
			var podNameArr []string
			json.Unmarshal([]byte(podNames), &podNameArr)
			var newPodNameArr []string
			if len(podNameArr) > 0 {
				for _, podName := range podNameArr {
					if podName != pod.Name {
						newPodNameArr = append(newPodNameArr, podName)
					}
				}
			}
			if len(newPodNameArr) != 0 {
				newPodNameByte, _ := json.Marshal(newPodNameArr)
				newPodNameStr := string(newPodNameByte)
				if podNames != newPodNameStr {
					methods.PutPodContentByEtcd(nodeName, strconv.Itoa(id), newPodNameStr)
				}
			} else {
				methods.DeletePodContentByEtcd(nodeName, strconv.Itoa(id))
			}
			methods.PutPodContentByEtcd(name, "complete", "1")
			c.schedulerCache.RemovePod(pod)
		} else {
...
func (c *Controller) updatePodInCache(oldObj, newObj interface{}) {
	...
	podUID := oldPod.UID
	// log.Printf("podUID is %+v", podUID)
	// log.Printf("oldPod.UID is %+v", oldPod.UID)

	// 1. Need update when pod is turned to complete or failed
	if c.schedulerCache.KnownPod(podUID) && utils.IsCompletePod(newPod) {
		log.Printf("Need update when pod is turned to complete or failed")
		needUpdate = true
	}
	...
	if !c.schedulerCache.KnownPod(podUID) && utils.GetGPUIDFromAnnotation(newPod) >= 0 {
		log.Printf("Need update when it's unknown pod, and GPU annotation has been set")
		needUpdate = true
	}
	// // 3. Need update when the batchsize has changed
	// if utils.GetTunedBatchsizeFromAnnotation(newPod) != utils.GetGPUIDFromAnnotation(oldPod) {
	// 	log.Printf("Need update when the batchsize has changed")
	// 	needUpdate = true
	// }
	if needUpdate {
