...
func NewSchedulerCache(nLister corelisters.NodeLister, pLister corelisters.PodLister, tuningQueue workqueue.RateLimitingInterface) *SchedulerCache {
...
func (cache *SchedulerCache) KnownPod(podUID types.UID) bool {
	cache.nLock.RLock()
	log.Printf("trace: CacheRWLocker KnownPod() Locked")
	defer log.Printf("trace: CacheRWLocker KnownPod() Unlocked")
...
func (cache *SchedulerCache) AddOrUpdatePod(pod *v1.Pod) error {
	...
	log.Printf("debug: Pod spec nodename is: %v", pod.Spec.NodeName)
...
// Get node
func (cache *SchedulerCache) GetNode(name string) (*v1.Node, error) {
	node, err := cache.nodeLister.Get(name)

	log.Printf("debug: Get gpuStatus: %v from node %v", node.GetAnnotations(), name)
	if err != nil {
		return nil, err
	}
	return node, nil
...
func (cache *SchedulerCache) GetNodeInfo(name string) (*NodeInfo, error) {
	node, err := cache.nodeLister.Get(name)
	log.Printf("debug: Get node info: %v", node.Name)
	if err != nil {
		return nil, err
	}
	cache.nLock.Lock()
	log.Printf("trace: CacheRWLocker GetNodeInfo() Locked")
	defer log.Printf("trace: CacheRWLocker GetNodeInfo() Unlocked")
	defer cache.nLock.Unlock()
	n, ok := cache.nodes[name]

	if !ok {
		n = NewNodeInfo(node, cache.tuningQueue)
		cache.nodes[name] = n
	} else {
...
func (cache *SchedulerCache) forgetPod(uid types.UID) {
	cache.nLock.Lock()
	log.Printf("trace: CacheRWLocker forgetPod() Locked")
	defer log.Printf("trace: CacheRWLocker forgetPod() Unlocked")
	defer cache.nLock.Unlock()
...
func (cache *SchedulerCache) rememberPod(uid types.UID, pod *v1.Pod) {
	cache.nLock.Lock()
	log.Printf("trace: CacheRWLocker rememberPod() Locked")
	defer log.Printf("trace: CacheRWLocker rememberPod() Unlocked")
	defer cache.nLock.Unlock()
